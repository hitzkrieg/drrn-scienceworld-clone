...
from vec_env_single_variation import resetWithVariation, initializeEnv, sanitizeInfo, sanitizeObservation
...
def evaluate_episode(agent, env, env_step_limit, simplificationStr, bufferedHistorySaverEval, extraSaveInfo, evalSet):
    ...
    ob = ""
    info = {}
    # There is no separate test / dev variation on which we evaluate. We evaluate on the 0th variation (Same as train). 
    if (evalSet == "dev"):
        ob, info = resetWithVariation(env, 0, 1, simplificationStr)
