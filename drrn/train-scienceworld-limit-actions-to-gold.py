...
filenameIn = "/home/hgolchha_umass_edu/ScienceWorld/goldpaths/gold_action_set.json"
f = open(filenameIn)
gold_actions_set_dict = json.load(f)
...
def load_gold_action_set(taskIdx, variationIdx, simplification_str='easy'):
    gold_action_set = gold_actions_set_dict[str(taskIdx)+ '_' + str(variationIdx)]
    if simplification_str == 'easy': 
        new_actions  = [] 
        for action in gold_action_set:
            if action[:len('go to')] == 'go to':
                new_actions.append('teleport to' + action[len('go to'):])
        
        gold_action_set =list(set(gold_action_set + new_actions))
...
def train(agent, envs, max_steps, update_freq, eval_freq, checkpoint_freq, log_freq, args, bufferedHistorySaverTrain, bufferedHistorySaverEval):
    ...
    valid_actions_list = [load_gold_action_set(args.task_idx, variationIdx) for variationIdx in variationIdxes] 
    ...
    for step in range(1, max_steps+1):
        ...
        for i, (done, info, new_variationIdx) in enumerate(zip(dones, infos, new_variationIdxes)):
            if new_variationIdx:
                ...
                valid_actions_list[i] = load_gold_action_set(args.task_idx, new_variationIdx)
        ...
        next_valids = [agent.encode(valid_actions) for valid_actions in valid_actions_list]
