...
def train(agent, envs, max_steps, update_freq, eval_freq, checkpoint_freq, log_freq, args, bufferedHistorySaverTrain, bufferedHistorySaverEval):
    ...
    for step in range(1, max_steps+1):
        ...
        for state, act, rew, next_state, valids, done, info in \
            zip(states, action_ids, rewards, next_states, next_valids, dones, infos):

            if args.sparse_rewards:
                if info['score']<100 or not done:
                    rew = 0
                else:
                    rew = info['score']
...
def parse_args():
    ...
    parser.add_argument('--sparse_rewards', default=0, type=int) # if 1, then a reward of 100 is obtained on correct task completion otherwise 0 always. 
