...
def resetWithVariation(env, variationMin, variationMax, simplificationStr):    
    ...
    return initialObs, initialDict, variationIdx
...
def initializeEnv(threadNum, args):    
    ...
    initialObs, initialDict, _ = resetWithVariation(env, 0, 1, args.simplification_str)
...
def worker(remote, parent_remote, threadNum, args):
    ...
    try:
        ...
        while True:
            ...
            if cmd == 'step':
                new_variationIdx = None
                if done:                    
                    # If the thread is done, reset it
                    print("Thread " + str(threadNum) + " is DONE -- resetting with new variation")                    
                    ob, info, new_variationIdx = resetWithVariationTrain(env, args.simplification_str)
                ...
                remote.send((ob, reward, done, info, new_variationIdx))
                
            elif cmd == 'reset':
                print ("------------------------------------ RESET (Thread " + str(threadNum) + ")")
                ob, info, variationIdx = resetWithVariationTrain(env, args.simplification_str)                
                info = sanitizeInfo(info)
                ob = sanitizeObservation(ob, info)
                
                remote.send((ob, info, variationIdx))
...
class VecEnv:
    ...
    def step(self, actions):
        ...
        # self.waiting = False
        obs, rewards, dones, infos, new_variationIdxes = zip(*results)
        return np.stack(obs), np.stack(rewards), np.stack(dones), infos, new_variationIdxes
    ...
    def reset(self):
        ...
        results = [remote.recv() for remote in self.remotes]
        obs, infos, variationIdxes = zip(*results)
        return np.stack(obs), infos, list(variationIdxes)
